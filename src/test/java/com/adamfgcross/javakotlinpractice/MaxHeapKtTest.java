...
public class MaxHeapKtTest {
    ...
    @DisplayName("Can create heap without throwing.")
    @Test
    ...
    @Test
    public void canGetHeapRoot() {
        MaxHeapKt maxHeapKt = new MaxHeapKt(arbitraryRoot);
        assertEquals(arbitraryRoot, maxHeapKt.getRoot());
    }
    ...
    public void heapOfThreeElementsMaintainsMaxHeapProperty() {
        ...
        assertEquals(6,maxHeapKt.getRoot());
    }
    ...
    public void heapOf100ElementsMaintainsMaxHeapProperty() {
        ...
        assertEquals(4967,maxHeapKt.getRoot());
    }

    @DisplayName("Given a heap of 100 Elements, Pick chooses first several in the right order")
    @Test
    public void givenAHeapOf100ElementsPickChoosesSeveralInRightOrder() {
        int[] elements = {19, 1146, 2530, 3054, 4566, 2093, 427, 3856, 3156, 1391, 3746, 3796, 1199, 4388, 4967, 4064, 3761, 2076, 3927, 2402, 2388, 4137, 4794, 2984, 3881, 1895, 498, 4824, 2548, 3004, 3673, 3507, 696, 1307, 3977, 3929, 1627, 3549, 3097, 553, 2740, 1366, 3570, 4112, 1295, 3160, 2036, 4488, 3498, 4374, 444, 106, 1556, 1994, 2526, 4249, 2719, 3392, 2205, 944, 1119, 3686, 1526, 2151, 1865, 1843, 1520, 1764, 3138, 2271, 1305, 983, 1758, 707, 4349, 1316, 2155, 1975, 2828, 1115, 8, 4550, 573, 1541, 1946, 2715, 2859, 4450, 4679, 2008, 3704, 4060, 4579, 239, 3981, 1704, 4450, 1733, 2540, 4582};
        MaxHeapKt maxHeapKt = new MaxHeapKt();
        for (int elt : elements) {
            maxHeapKt.add(elt);
        ...
        int[] pickedElements = new int[expectedElements.length];
        for (int i = 0; i < expectedElements.length; i++) {
            pickedElements[i] = maxHeapKt.pick();
        }
        assertArrayEquals(expectedElements,pickedElements);
    }

    @DisplayName("Given a heap of 100 Elements, Pick chooses first several in the right order (2)")
    @Test
    public void givenAHeapOf100ElementsPickChoosesSeveralInRightOrder2() {
